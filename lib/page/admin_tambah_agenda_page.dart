...
class AdminTambahAgendaPage extends StatefulWidget {
  const AdminTambahAgendaPage({super.key});

  @override
  State<AdminTambahAgendaPage> createState() => _AdminTambahAgendaPageState();
}

class _AdminTambahAgendaPageState extends State<AdminTambahAgendaPage> {
  ...
  Future<void> _handleSave() async {
    if (_namaAgendaController.text.isEmpty ||
        _tanggalController.text.isEmpty ||
        _selectedPjWarga == null ||
        _selectedKasId == null) {
      _showSnackbar("Mohon lengkapi data (Nama, Tanggal, PJ, dan Kas)!", Colors.red);
      return;
    }
    ...
    try {
      final agendaVM = Provider.of<AgendaViewmodel>(context, listen: false);
      
      final newAgenda = AgendaModel.Data(
        namaAgenda: _namaAgendaController.text,
        tanggal: _tanggalController.text,
        jamMulai: _jamMulaiController.text,
        lokasi: _lokasiController.text,
        penanggungJawab: AgendaModel.PenanggungJawabWarga(
          id: _selectedPjWarga?.id,
          nama: _selectedPjWarga?.nama,
        ),
      ...
      await agendaVM.createAgenda(newAgenda);

      _showSnackbar("Agenda berhasil ditambahkan!", const Color(0xFF5CB85C));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        centerTitle: true,
        title: const Text('Tambah Agenda', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        leading: IconButton(
          icon: const Icon(Icons.arrow_back, color: Colors.white), // Back putih
          onPressed: () => Navigator.pop(context),
        ),
      ),
      body: Container(
        width: double.infinity,
        height: double.infinity, // Panjang sampai bawah
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    _buildInputField("Nama Agenda", _namaAgendaController, hint: "Contoh: Kerja Bakti"),
                    ...
                    _buildInputField("Lokasi", _lokasiController, hint: "Contoh: Balai Desa"),
                    const SizedBox(height: 20),
                    
                    _buildWargaSearchField(), // Search PJ Warga
                    const SizedBox(height: 20),

                    _buildKasDropdown(), // Select Data Kas
                    const SizedBox(height: 20),
        ...
        Consumer<KasViewModel>(
          builder: (context, vm, _) {
            return DropdownButtonFormField<int>(
              dropdownColor: Colors.white, // Dropdown sheet putih
              decoration: InputDecoration(
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
    ...
    return Container(
      ...
      child: ElevatedButton(
        ...
        child: _isSubmitting
            ...
            : const Text("Simpan Agenda", style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
      ),
    );
  }
}